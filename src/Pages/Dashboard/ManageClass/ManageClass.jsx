...
const ManageClass = () => {
    ...
    const handleApproved = pendingClass => {
        ...
        fetch(` https://language-server-brown.vercel.app/pendingClass/approved/${pendingClass._id}`, {
            method: 'PATCH'
        })
            .then(res => res.json())
            .then(data => {
                ...
                if (data.modifiedCount) {
                    fetch(' https://language-server-brown.vercel.app/course', {
                        method: 'POST',
                        headers: {
    ...
    const handDenied = id => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(` https://language-server-brown.vercel.app/pendingClass/${id}`, {
                    method: 'DELETE'
                })
                    .then(res => res.json())
                    .then(data => {
                    ...
                    <tbody>
                        {/* row 1 */}
                        {
                            ...
                            >
                                ...
                                <td>
                                    {
                                        pendingClass.status === 'Approved' ?
                                            ...
                                            :
                                            <button onClick={() => handleApproved(pendingClass)} className="btn bg-teal-600 btn-sm text-white">Approved</button>
                                    }
                                ...
                                <th>
                                    <button onClick={() => handDenied(pendingClass._id)} className="btn bg-teal-600 btn-sm text-white">Denied</button>
                                </th>
                            </tr>)
                        }
