...
const CategoryTab = ({ data }) => {
    ...
    const { courseName, image, price, rating, Length, _id, instractorName } = data;
        ...
        <div className="card w-74 bg-base-100 border border-slate-400">
            ...
            <div className="card-body ">
                ...
                <div className="flex justify-center gap-10">
                    <p>Rating:{rating}</p>
                    <p className="font-bold text-teal-600 ">${price}</p>
                </div>
                <p className=" text-center py-2 border-t-4 border-teal-600">Length : {Length}</p>
                <button onClick={() => handleSelect(data)} className="btn text-yellow-300 bg-teal-600 font-bold btn-active btn-sm ">Add To Cart</button>
            </div>
        </div>
    );
};
